...
Creation date: 22/9/2025

Script to handle computing the areal cloud fraction for all collocation events between ICESat-2 and Cloudnet for a given collocaiton event
"""
...
def parse_args() -> None | Args:
    import argparse
    parser = argparse.ArgumentParser(description="Compute ACFs for ATL09 and CloudNet data")
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='Directory path to location for saving output netcdf files'
    )
...
def pair_acf_dataset_from_collocated_homogenised_data(coll_H):
    """From two acf.ACF instances, join the [RawATL09 |  RawCloudnet] elements in a single xarray dataset.
    A deep copy is made before returning so that no stray references to coll_H remain.
    """
...
def main(args: Args):
    ...
    fname_out = f"acfs-per-event_{args.site}_{R_km:3.3f}km_{tau_s:06}s"
    ...
    # this time, I ask for no forgiveness
    events_to_load = (
        (
            event, 
            print("\n",f"LOADING",event)
    ...
    collocated_homogenised_data_and_n_profiles = filter_none((
        ...
        if (
            ...
            None,
            print(type(collocated_h[0]), collocated_h[0]), # make_safe message
    ...
    # create acf data arrays from succesfully homogenised data
    N_success = 0
    n_profiles = {
        RawATL09: list(),
        RawCloudnet: list(),
    ...
    paired_acfs = (
        (
            pair_acf_dataset_from_collocated_homogenised_data(coll_H),
    ...
    acfs_per_event = xr.concat(
        paired_acfs,
        dim="collocation_event"
    )
    acfs_per_event["n_profiles_atl09"] = ( ("collocation_event",) , n_profiles[RawATL09] )
    acfs_per_event["n_profiles_cloudnet"] = ( ("collocation_event",) , n_profiles[RawCloudnet] )

    print(f"SAVING TO {fpath_out=}")
    acfs_per_event.to_netcdf(fpath_out)
